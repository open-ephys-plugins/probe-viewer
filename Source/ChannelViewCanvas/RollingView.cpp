...
#pragma mark - RollingView -

RollingView::RollingView (ChannelViewCanvas* canvas) : isDirty (false), numPixelUpdates (0), canvas (canvas), channelHeight (10), screenBufferImage (Image::RGB, CHANNEL_DISPLAY_WIDTH, CHANNEL_DISPLAY_MAX_HEIGHT * 384, false, SoftwareImageType()), frontBackBufferPixelOffset (0), frontBufferIndex (0), fullRedraw (false), numChannels (0), windowSize (ProbeViewerCanvas::TRANSPORT_WINDOW_TIMEBASE)
{
}

void RollingView::paint (Graphics& g)
{
...
void RollingView::updateViewSettings()
{
    ...
    if (numChannels > 0)
    {
        ...
        int numTiles = ceil (RollingView::CHANNEL_DISPLAY_WIDTH / float (RollingView::CHANNEL_DISPLAY_TILE_WIDTH));
...
void RollingView::tick()
{
    if (++frontBackBufferPixelOffset >= CHANNEL_DISPLAY_TILE_WIDTH)
    {
        ...
        frontBufferIndex += 1;
        if (frontBufferIndex >= displayBitmapTiles.size())
            frontBufferIndex = 0;
    }
}

void RollingView::setChannelHeight (float height)
{
...
float RollingView::getChannelHeight()
{
...
BitmapRenderTile* const RollingView::getFrontBufferPtr() const
{
    return displayBitmapTiles[frontBufferIndex];
}

int RollingView::getBufferOffsetPosition() const
{
    return frontBackBufferPixelOffset;
}

#pragma mark - RollingView Constants

const int RollingView::CHANNEL_DISPLAY_MAX_HEIGHT = 2; // more efficient than scaling down, but slightly lossy in definition (cross channel bleed)
const int RollingView::CHANNEL_DISPLAY_WIDTH = 1920;
const int RollingView::CHANNEL_DISPLAY_TILE_WIDTH = 64;
const Colour RollingView::backgroundColour (0, 18, 43);
...
ProbeChannelDisplay::ProbeChannelDisplay (RollingView* view_,
                                          CanvasOptionsBar* optionsBar_,
                                          int channelID_,
                                          float sampleRate_)
    : rollingView (view_), optionsBar (optionsBar_), sampleRate (sampleRate_), channelID (channelID_)
{
...
{
    RenderMode rm = rollingView->canvas->getCurrentRenderMode();

    Image bdSubImage (rollingView->getFrontBufferPtr()->getChannelSubImage (channelID));
    ...
    if (rm == RenderMode::RMS)
    {
        ...
        if (boundSpread == 0)
            boundSpread = 1;
        const auto val = (samples[samples.size() - rollingView->numPixelUpdates] - optionsBar->getRMSLowBound()) / boundSpread;

        Colour colour = ColourScheme::getColourForNormalizedValueInScheme (val, rollingView->canvas->getCurrentColourScheme());

        const int xPix = rollingView->getBufferOffsetPosition();
    ...
    else if (rm == RenderMode::SPIKE_RATE)
    {
        ...
        if (boundSpread == 0)
            boundSpread = 1;
        const auto val = (samples[samples.size() - rollingView->numPixelUpdates] - optionsBar->getSpikeRateLowBound()) / boundSpread;
    ...
    else
    {
        ...
        if (boundSpread == 0)
            boundSpread = 1;
        const auto val = (samples[samples.size() - rollingView->numPixelUpdates] - optionsBar->getFFTLowBound()) / boundSpread;
    ...
    if (rollingView->numPixelUpdates == 1)
    {
        samples.clear();
    }
}
